 ...
 * @returns {{ byDate: Map<string, Array<{content, hours, source, weeklyPlanId, planItemIndex, order}>>, errors: string[] }}
 */
  ...
  for (const { item, idx } of sortedItems) {
    ...
    if (isZeroHour) {
      ...
      for (const date of availableDates) {
        ...
        if (dueDateStr && date > dueDateStr) break; // 마감 지난 날짜엔 배치 금지
 ...
 */
async function applyDistributionToChecklists(userId, weeklyPlanId, byDate) {
  const dates = [...byDate.keys()];
  if (dates.length === 0) return;

  await DailyChecklist.bulkWrite(dates.map(date => ({
    updateOne: {
      filter: { userId, date },
